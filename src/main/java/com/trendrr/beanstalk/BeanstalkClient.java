...
public class BeanstalkClient {
	...
	public Long tubeReadyCount(String tube) throws BeanstalkException{
		String result = tubeStats(tube);
		if (result!=null) {
			for(String data: result.split("\n")){
				String[] keyvalue = data.split(":");
				if ("current-jobs-ready".equals(keyvalue[0].trim())) {
					return Long.valueOf(keyvalue[1].trim());
				}
			}
		}
		return null;
